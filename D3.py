...
iters = {
    'U': lambda p, dist: iter((p[0], p[1]-i) for i in range(1, dist+1)),
    'R': lambda p, dist: iter((p[0]+i, p[1]) for i in range(1, dist+1)),
    'D': lambda p, dist: iter((p[0], p[1]+i) for i in range(1, dist+1)),
    'L': lambda p, dist: iter((p[0]-i, p[1]) for i in range(1, dist+1)),
}
...
for wire_index, wire in enumerate(d3_input):
    ...
    for move in wire:
        mdir, mdist = move[0], int(move[1:])
        ox, oy = cpos
        for x, y in iters[mdir](cpos, mdist):
            rx = x+maxx
            ry = y+maxy
            if bitmap[ry][rx] != '#' and bitmap[ry][rx] != str(wire_index):
                min_distance = min(min_distance, get_distance((0, 0), (x, y)))
...
print('D3P1 result:', min_distance)
...
min_length = min(map(lambda i: i[0]+i[1], intercept_mapping.values()))

# img.show()

print('D3P2 result:', min_length)
